...
del /q "%_JAVM_ENV_FILE%" >nul 2>&1
rmdir "%_JAVM_TEMP_DIR%" >nul 2>&1
endlocal & set "PATH=%_JAVM_NEW_PATH%" & set "JAVA_HOME=%_JAVM_NEW_JAVA_HOME%" & set "JAVA_HOME_BEFORE_JAVM=%_JAVM_NEW_JAVA_HOME_BEFORE_JAVM%" & exit /b %_JAVM_EXIT_CODE%

:javm_apply_deactivate
del /q "%_JAVM_ENV_FILE%" >nul 2>&1
rmdir "%_JAVM_TEMP_DIR%" >nul 2>&1
endlocal & set "PATH=%_JAVM_NEW_PATH%" & set "JAVA_HOME=%_JAVM_NEW_JAVA_HOME%" & set "JAVA_HOME_BEFORE_JAVM=" & exit /b %_JAVM_EXIT_CODE%
